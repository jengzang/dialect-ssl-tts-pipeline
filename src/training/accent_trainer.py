...
class AccentTrainer(BaseTrainer):
    """口音识别训练器
    ...
    def __init__(
        ...
        self.optimizer = optim.Adam(
            ...
            lr=self.wav2vec_config.get('learning_rate', 1e-4)
        )
    ...
    def train(
        ...
        **kwargs
        ...
        Returns:
            训练历史字典
        """
        epochs = epochs or self.wav2vec_config.get('epochs', 10)
        patience = self.wav2vec_config.get('early_stopping_patience', 5)

        # 初始化早停
        early_stopping = EarlyStopping(patience=patience, mode='min')

        self.logger.info("=" * 60)
        self.logger.info("开始训练口音分类器")
        self.logger.info("=" * 60)
        self.logger.info(f"训练轮数: {epochs}")
        self.logger.info(f"批次大小: {train_loader.batch_size}")
        self.logger.info(f"学习率: {self.wav2vec_config.get('learning_rate', 1e-4)}")
